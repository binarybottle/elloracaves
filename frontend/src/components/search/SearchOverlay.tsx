...
export default function SearchOverlay({ onClose, onImageSelect }: SearchOverlayProps) {
  ...
  const parseQuery = (q: string) => {
    ...
    return { query: parsedQuery, cave: caveFilter };
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const { query: parsedQuery, cave } = parseQuery(query);
        const caveId = cave ? parseInt(cave, 10) : undefined;
        
        const data = await searchImages(parsedQuery, caveId, page);
  ...
  const handleImageClick = async (result: any) => {
    ...
    try {
      const imageData = await fetchImageDetail(image.id);
      onImageSelect(
        imageData.cave_id,
      ...
      onImageSelect(image.cave_id, 1, image.id);
      onClose();
    }
  };
      ...
      <div className="w-full max-w-5xl bg-black border-2 border-gray-700 rounded-lg shadow-2xl mt-8 mb-8">
        ...
        <div className="p-4 border-b border-gray-700">
          ...
          <div className="mt-2 text-xs text-gray-400">
            <span className="font-semibold">Advanced:</span> <code className="bg-gray-800 px-1 rounded">cave:16</code>, <code className="bg-gray-800 px-1 rounded">subject:Buddha</code> • Fuzzy matching enabled
          </div>
        </div>
